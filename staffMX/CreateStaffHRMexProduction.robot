...
${OTP_INPUT_FIELD_XPATH}   xpath=//input[contains(@class, 'input_code')]
${OTP_FIRST_FIELD}         id=input1
${OTP_CONTAINER}           id=otp-container
${otp_code}                777777
...
*** Variables ***
# Position Selection Locators
${POSITION_DROPDOWN}          xpath=//span[@id='select2-position_id-container']
${POSITION_ARROW}             xpath=//span[contains(@class,'select2-selection__arrow')]
${POSITION_OPTIONS}           xpath=//ul[contains(@class,'select2-results__options')]
${POSITION_EXECUTIVE}         xpath=//li[contains(@class,'select2-results__option') and contains(text(),'EXECUTIVE VICE PRESIDENT')]
${POSITION_TESTING}           xpath=//li[contains(@class,'select2-results__option') and contains(text(),'testing')]
${SUCCESS_INDICATOR}          xpath=//span[@id='select2-position_id-container' and not(contains(text(),'- Select Position -'))]
...
${Taxno_Value}             9909901355  
${Curp_FIELD}              id=curp
${Curp_Value}              9909901355  
${TaxAdd_FIELD}            id=tax_zipcode                    
${TaxAdd_Value}            9909901355
${Insuno_FIELD}            id=insurance_number                
${Insuno_Value}            9909901355
${IDcard_FIELD}            id=id_card                
${IDcard_Value}            9909901355    
${BankAcc_FIELD}           id=bank_account                              
${BankAcc_FValue}          9909901355
${BankNo_FIELD}            id=bank_account_number                  
${BankNo_Value}            9909901355
...
${ExNowork_Value}          222227
${InNowork_Field}          id=internal_work_location
${InNowork_Value}          222227
...
${EmerContPhone_Value}     0944222227
${File1}                   id=file1
${File2}                   id=file2
${File3}                   id=file3
${File4}                   id=file4
...
กรอก OTP ทีละช่องเหมือนมนุษย์
    [Documentation]    กรอก OTP ลงในช่องทั้ง 6 โดยใช้กลยุทธ์หลายแบบ
    Wait Until Element Is Visible    ${OTP_INPUT_FIELD_LIST}    10s
    ${otp_fields_count}=    Get Element Count    ${OTP_INPUT_FIELD_LIST}
    Log    พบ OTP fields จำนวน: ${otp_fields_count} ช่อง
    
    # วิธีที่ 1: ลองใช้ CSS selector แบบเดิมก่อน
    Run Keyword And Ignore Error    กรอก OTP ด้วยวิธีที่ 1
    
    # วิธีที่ 2: ถ้าวิธีที่ 1 ล้มเหลว ลองใช้ XPath
    Run Keyword And Ignore Error    กรอก OTP ด้วยวิธีที่ 2
    
    # วิธีที่ 3: ลองใช้การค้นหาแบบอื่น
    Run Keyword And Ignore Error    กรอก OTP ด้วยวิธีที่ 3
    
    # ตรวจสอบว่ากรอก OTP สำเร็จหรือไม่
    ตรวจสอบและกดส่ง OTP

กรอก OTP ด้วยวิธีที่ 1
    [Documentation]    ใช้ CSS selector แบบเดิม
    @{otp_fields}=    Get WebElements    ${OTP_INPUT_FIELD_LIST}
    FOR    ${index}    ${field}    IN ENUMERATE    @{otp_fields}
        Run Keyword And Ignore Error    Scroll Element Into View    ${field}
        Run Keyword And Ignore Error    Click Element    ${field}
        Run Keyword And Ignore Error    Input Text    ${field}    7
        Sleep    0.3s
    END

# กรอก OTP ด้วยวิธีที่ 2
#     [Documentation]    ใช้ XPath เพื่อหา input fields
#     ${otp_xpath}=    Set Variable    xpath=//input[contains(@class, 'input_code') and contains(@name, 'otp_code')]
#     @{otp_fields}=    Get WebElements    ${otp_xpath}
#     FOR    ${index}    ${field}    IN ENUMERATE    @{otp_fields}
#         Run Keyword And Ignore Error    Scroll Element Into View    ${field}
#         Run Keyword And Ignore Error    Click Element    ${field}
#         Run Keyword And Ignore Error    Input Text    ${field}    7
...
# กรอก OTP ด้วยวิธีที่ 3
#     [Documentation]    ใช้การค้นหาโดย ID และ class ร่วมกัน
#     # ลองหาช่องแรกด้วย ID
#     Run Keyword And Ignore Error    Input Text    id=input1    7
#     Sleep    0.3s
    
#     # หาช่องที่เหลือด้วย class
#     ${other_otp_fields}=    Get WebElements    xpath=//input[contains(@class, 'input_code') and not(@id)]
#     FOR    ${field}    IN    @{other_otp_fields}
#         Run Keyword And Ignore Error    Scroll Element Into View    ${field}
#         Run Keyword And Ignore Error    Click Element    ${field}
#         Run Keyword And Ignore Error    Input Text    ${field}    7
#         Sleep    0.3s
...
ตรวจสอบและกดส่ง OTP
    [Documentation]    ตรวจสอบว่าปุ่มส่งเปิดใช้งานแล้วจึงกด
    # รอให้ปุ่มส่งเปิดใช้งาน
    Wait Until Keyword Succeeds    20s    1s    Element Should Be Enabled    ${OTP_SUBMIT_BUTTON}
    
    # ลองกดปุ่มส่งหลายวิธี
    Run Keyword And Ignore Error    Click Button    ${OTP_SUBMIT_BUTTON}
    Run Keyword And Ignore Error    Click Element    ${OTP_SUBMIT_BUTTON}
    Run Keyword And Ignore Error    Js Click    ${OTP_SUBMIT_BUTTON}
    
    # ตรวจสอบผลลัพธ์
    Wait Until Keyword Succeeds    15s    1s    ตรวจสอบผล OTP

ตรวจสอบผล OTP
    [Documentation]    ตรวจสอบว่าผ่าน OTP หรือไม่
    ${success_visible}=    Run Keyword And Return Status    Wait Until Element Is Visible    ${SUCCESS_MODAL}    5s
    Run Keyword If    ${success_visible}    Click Button    ${SUCCESS_OK_BUTTON}
    
    # หรือตรวจสอบว่าไปถึงหน้าถัดไป
    ${next_page}=    Run Keyword And Return Status    Wait Until Element Is Visible    ${STAFF_MENU}    50s
    Run Keyword Unless    ${success_visible} or ${next_page}    Fail    ไม่สามารถผ่านหน้า OTP ได้
...
Navigate To Create Staff
    Wait Until Element Is Visible    ${STAFF_MENU}    20s
...
Select Organization And Position
    [Documentation]    เลือกองค์กรและตำแหน่งงาน - รองรับทั้ง test และ production
    Log To Console    Selecting Organization and Position...
    ...
    Wait Until Element Is Visible    xpath=//span[contains(@class, 'select2-selection--szingle')]    timeout=10s
    Click With Fallback    xpath=//span[contains(@class, 'select2-selection--single')]
    Sleep    1.5s

Select Position
    [Documentation]    เลือกตำแหน่งงาน - ลองทีละวิธีจนสำเร็จ
    Wait Until Keyword Succeeds    3x    2s    ลองเลือกตำแหน่งจนสำเร็จ

ลองเลือกตำแหน่งจนสำเร็จ
    [Documentation]    ลองทุกวิธีจนกว่าจะเลือกตำแหน่งสำเร็จ
    # ลองวิธีที่ 1
    Run Keyword And Ignore Error    คลิกและเลือกตำแหน่งโดยตรง
    
    # ลองวิธีที่ 2
    Run Keyword And Ignore Error    คลิกลูกศรและเลือกตำแหน่ง
    
    # ลองวิธีที่ 3  
    Run Keyword And Ignore Error    ใช้JavaScriptเลือกตำแหน่ง
    
    # ตรวจสอบผลลัพธ์
    ตรวจสอบการเลือกตำแหน่งสำเร็จ

คลิกและเลือกตำแหน่งโดยตรง
    Click Element    xpath=//span[@id='select2-position_id-container']
    Wait Until Element Is Visible    xpath=//ul[contains(@class,'select2-results__options')]    5s
    Click Element    xpath=//li[contains(text(),'EXECUTIVE VICE PRESIDENT')]

คลิกลูกศรและเลือกตำแหน่ง  
    Click Element    xpath=//span[contains(@class,'select2-selection__arrow')]
    Wait Until Element Is Visible    xpath=//input[contains(@class,'select2-search__field')]    5s
    Input Text    xpath=//input[contains(@class,'select2-search__field')]    EXECUTIVE VICE PRESIDENT
    Click Element    xpath=//li[contains(text(),'EXECUTIVE VICE PRESIDENT')]

ใช้JavaScriptเลือกตำแหน่ง
    Execute JavaScript    document.querySelector('select[name="position_id"]').click();
    Execute JavaScript    var options = document.querySelectorAll('select[name="position_id"] option');
    for(var i=0; i<options.length; i++) {
        if(options[i].textContent.includes('EXECUTIVE VICE PRESIDENT')) {
            options[i].selected = true;
            var event = new Event('change', {bubbles: true});
            document.querySelector('select[name="position_id"]').dispatchEvent(event);
            break;
        }
    }

ตรวจสอบการเลือกตำแหน่งสำเร็จ
    [Documentation]    ตรวจสอบว่าการเลือกตำแหน่งสำเร็จ
    Wait Until Element Is Not Visible    xpath=//span[contains(@class,'select2-container--open')]    10s
    ${current_position}=    Get Text    xpath=//span[@id='select2-position_id-container']
    Should Not Contain    ${current_position}    - Select Position -
    Log    เลือกตำแหน่งสำเร็จ: ${current_position}
...
Fill File 1
    ...
    Wait Until Element Is Visible    ${File1}    2s
...
Fill File 2
    ...
    Wait Until Element Is Visible    ${File2}    2s
...
Fill File 3
    ...
    Wait Until Element Is Visible    ${File3}    2s
...
Fill File 4
    ...
    Wait Until Element Is Visible    ${File4}    2s
...
Fill File 5
    ...
    Wait Until Element Is Visible    ${File5}    2s
...
Fill File 6
    ...
    Wait Until Element Is Visible    ${File6}    2s
...
Fill File 7
    ...
    Wait Until Element Is Visible    ${File7}    2s
...
Fill File 8
    ...
    Wait Until Element Is Visible    ${File8}    2s
...
Fill File 9
    ...
    Wait Until Element Is Visible    ${File9}    2s
...
Fill File 10
    ...
    Wait Until Element Is Visible    ${File10}    2s
...
Fill File 11
    ...
    Wait Until Element Is Visible    ${File11}    2s
...
Fill File 12
    ...
    Wait Until Element Is Visible    ${File12}    2s
...
Click Next Button Payment Page
    Scroll Element Into View    ${Next_Docs}
    Wait Until Element Is Visible    ${Next_Docs}    timeout=10s
    Click Element    ${Next_Docs}
    Log To Console    Clicked the Next button
    Sleep    4s
...
Agent Joint Date
    Log To Console    Waiting 1.5s For Process..
    Sleep    1.5s
    # Scroll ไปที่ฟิลด์ Joint Data Fill
